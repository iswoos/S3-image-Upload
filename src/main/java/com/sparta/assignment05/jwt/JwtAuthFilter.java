...
public class JwtAuthFilter extends OncePerRequestFilter {
    // jwtUtil 에서 만들어 놓은 토큰 관련 메서드들 가져다가 필터만듦
    private final JwtUtil jwtUtil;

    // doFilter( ) 메소드는 필터링 설정한 서블릿을 실행할 때마다 호출되는 메소드로서 실제 필터링 기능을 구현하는 메소드
    // doFilterInternal() doFilter 전에 끼어들은 커스텀한 Filter
    @Override
    ...
    public void jwtExceptionHandler(HttpServletResponse response, String msg, HttpStatus status) {
        ...
        try {
            String json = new ObjectMapper().writeValueAsString(new ExceptionDto(msg, status.value()));
